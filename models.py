...
class NearEarthObject:
    ...
    def __init__(self, ppdes, nname='None', ndiameter='nan', hazardous='N',
                 **info):
    ...
    def __str__(self):
        ...
        return f"NearEarthObject ***{self.designation!r} aka {self.name!r},"\
            f" has a diameter of {self.diameter:.3f} Hazardous:\
                {self.hazardous!r}"
...
class CloseApproach:
    ...
    def __init__(self, designation, ccd=None, ddist=0.0, vv_rel=0.0, **info):
        ...
        :param designation: A String Primary designation of the asteroid or
        comet (e.g., 443, 2000 SG344)
        :param ccd (time) time of close-approach (formatted calendar date/time,
                                                  in UTC)
        :param ddist (float)  nominal approach distance (au)
        :param vv_rel (float) velocity relative to the approach body at close
        approach (km/s)
        :param info: A dictionary of excess keyword arguments supplied to the
        constructor.
        """
        ...
        if ccd != '':
            self.time = cd_to_datetime(ccd)  # Use the cd_to_datetime
    ...
    def __str__(self):
        ...
        # The project instructions include one possibility. Peek at the
        # __repr__ method for examples of advanced string formatting.
        return f"A CloseApproach at {self.time_str!r} located at distance\
            {self.distance:.2f}," f"with {self.velocity:.2f}\
                and neo{self.neo!r})"
